...
The html version is available at [Practical Machine Learning](http://ruishu.github.io/Practical_Machine_Learning)
...
```{r eval=FALSE, create_model_output}
...
Cross-validation was performed using the previously partitioned testing data. Out-of-sample error is estimated to be 1-0.995 = 0.005, a respectable value.
...
```
```{r eval=FALSE, cross_validation_output}
...
After cleaning the [test data](https://d396qusza40orc.cloudfront.net/predmachlearn/pml-testing.csv) in the same manner as the training data (described above), predictions were made based on the training model:

```{r eval=FALSE, predicting}
